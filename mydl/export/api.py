...
from mydl.config import CfgNode as CN
...
def export_caffe2_model(cfg, model, inputs):
    """
    Export a mydl model to caffe2 format.

    Args:
        cfg (CfgNode): a mydl config, with extra export-related options
            added by :func:`add_export_config`.
        model (nn.Module): a model built by
            :func:`mydl.modeling.build_model`.
...
def export_onnx_model(cfg, model, inputs):
    """
    Export a mydl model to ONNX format.
...
class Caffe2Model(nn.Module):
    ...
    def __call__(self, inputs):
        """
        An interface that wraps around a caffe2 model and mimics mydl's models'
